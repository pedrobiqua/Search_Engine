...
public:
   ...
   * each vertex in the graph, iterating until the maximum number of iterations
   * is reached or the variation is smaller than the specified tolerance.
   *
   * @param damping_factor Damping factor used in the PageRank calculation. Default value is 0.85.
   * @param max_iterations Maximum number of iterations for the algorithm. Default value is 100.
   * @param tol Tolerance for the convergence of the algorithm. Default value is 1e-6.
   * @param on_print Flag to control if the iterations should be printed. Default value is `false`.
   * @return A map containing the PageRank of each vertex, where the key is the vertex and the value is the PageRank.
   */
...
private:
   ...
   *
   * The graph is represented by a map, where the keys are the vertices and the
   * values are lists of integers representing the adjacent vertices.
   */
