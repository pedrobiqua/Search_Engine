...
TEST(InvertedIndexTest, AddAndFindDocuments) {
    // Map onde será armazenado os índices e suas respectivas listas de documentos
    inverted_index::map_str_docs mp;

    // Documentos de exemplo
    std::vector<std::pair<std::string, std::string>> documents = {
        {"Albert Einstein", "A imaginação é mais importante que o conhecimento, pois o conhecimento é limitado."},
        {"Isaac Newton", "Se vi mais longe foi por estar sobre ombros de gigantes."},
        {"René Descartes", "Penso, logo existo. É a base do conhecimento humano."},
        {"Aristóteles", "A felicidade depende de nós mesmos."},
        {"Sócrates", "Só sei que nada sei, mas em reconhecer minha ignorância, abro as portas para o aprendizado."},
        {"Leonardo da Vinci", "A simplicidade é o último grau de sofisticação."},
        {"Galileu Galilei", "Eppur si muove."},
        {"Confúcio", "Aquele que move montanhas começa carregando pequenas pedras."}
    };
    ...
    EXPECT_TRUE(std::any_of(docs.begin(), docs.end(), [](const inverted_index::docs& d) {
