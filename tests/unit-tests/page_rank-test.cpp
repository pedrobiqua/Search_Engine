...
TEST(PageRankTest, AddEdge) {
    ...
    std::map<int, std::list<int>> actual = g.get_adjList(); // Necessita de um getter
...
TEST(PageRankTest, PageRankConvergence) {
    ...
    // Como o grafo é bidirecional e balanceado, o PageRank deve ser igual para ambos os nós
...
TEST(PageRankTest, LargerGraph) {
    ...
    // Não esperamos resultados exatos aqui, mas podemos verificar se os valores fazem sentido
